  ...
  $(document).delegate('.btn-sve', 'click', function(){
    ...
    if(mainform_ID == 0 && subformID == 0)
    {
      ...
      if (daysDifference == 0)
      {
        GettingShiftAllocation(client_id,carer_id).then(AllocationResponse=>{
          console.log("AllocationResponse",AllocationResponse);
          if(AllocationResponse == 0){
            Swal.fire({
              title: "ShiftAllocation is not done for this carer",
              icon: "warning",
              showConfirmButton :false,
              timer: 3000
            });
            ...
            var start_date = AllocationResponse[0].Start_Date;
            var End_date = AllocationResponse[0].End_date;
            if(dateCheck(start_date,End_date,Shift_start))
            {
              CreateEmpShiftFunc(formData).then(empshiftResp=>{
                ...
                $(".EMP_tr1").remove();
                $(".empty").remove();
                FilterFuncSP(carernameValues,clientnameValues,carefaciValues);
            ...
            else
            {
              $(".btn-sve").prop("disabled",false);
              Swal.fire({
                title: "Sorry, these dates are not available for shift allocation",
                icon: "warning",
                showConfirmButton :false,
                timer: 3000
              });
      ...
      { 
        var s_date = moment(Shift_start).format("YYYY-MM-DD");
        var e_date = moment(Shift_end).format("YYYY-MM-DD");
        ...
        GettingShiftAllocation(client_id,carer_id).then(AllocationResponse=>{
          ...
          if(AllocationResponse == 0)
          {
            ...
            var i=0;
            var datelength = datesInRangeFormatted.length;
            const NotAvailableDates = [];
            for(i=0;i < datelength;i++ )
            {
              datecheck = datesInRangeFormatted[i];
              if(dateCheck(start_date,End_date,datecheck))
              var Message = "Availed";
              else
              {
                NotAvailableDates.push(datecheck);
              }
            }
            console.log( "k" +NotAvailableDates.length);
    ...
    else if (daysDifference > 0)
    {
      ...
      GettingShiftAllocation(client_id,carer_id).then(AllocationResponse=>{
        ...
        if(AllocationResponse == 0)
        {
          ...
          console.log( "not" +NotAvailableDates.length);
          if(NotAvailableDates.length > 0)
          {
            $(".btn-sve").prop("disabled",false);
            Swal.fire({
